...
**Theme** (all optional): `accent` (one colour for halo, click effect and highlights), `cursorStyle` (`arrow` · `mac` · `hand` · `ibeam` · `dot` · `auto` = hand over links/buttons, I-beam over text fields), `cursorSize` (34), `halo` (true), `haloStyle` (`fill` · `ring` · `glow`), `haloSize`, `haloFill`, `haloStroke`, `clickStyle` (`ripple` · `ring` · `pulse` · `none`), `caret` (true: the text caret is drawn on the virtual clock — solid while typing, then a steady blink; `false` keeps Chromium's, which flickers at random in the video), `cursorMotion` (`{ spring, arc, blur }`, all on; `false` turns all off — cubic ease, straight line, no blur), `autoZoom` (on by default; `false` turns it off, or `{ scale, ms, gap, dwell, minHold, clicks }` — defaults 1.35, 1400, 10000, 4000, 1500, `"smart"`), `captionPosition`, `captionSize`, `badgeTop`, `font`, `keycaps` (`mac` default: `Control`/`Meta` → ⌘, ⌥ ⇧ ↩; `win`: Ctrl, Alt, Shift; `false` hides them), `seed` (a number: `Math.random` becomes a seeded PRNG, so apps with random demo data give identical takes).
...
**Auto-zoom** (default): a few gentle zooms without writing any — when typing starts (onto the field), and on a click in the page body followed by a hold ≥ 1.5 s. Clicks are `"smart"` by default: the recorder watches what the page changes in the 300 ms after the click and zooms onto click + change only when that sits in the body and covers < 35 % of the view — a side panel sliding in at the edge, a new page or no visible change means no zoom. `clicks: true` always zooms onto the click, `false` never. At most one per 10 s, 1.35×, slow; out again after 4 s or before a long cursor move. Clicks near the edges (toolbars, nav) never zoom — their result shows elsewhere. The first manual `d.zoom()` hands the camera to the scenario for the rest of the take.
...
- **Zoom sparingly**: auto-zoom covers typing and click-then-hold when the result lands near the click; add a manual `d.zoom` only where it misses (it then takes over). One or two zooms per minute, 1.3–1.5×, on the moment that matters (a value changing, small text). Zoom out before a big move across the screen. A highlight is often enough.
